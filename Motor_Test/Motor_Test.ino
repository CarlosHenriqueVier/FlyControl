...
#define SEA_LEVEL_PRESSURE (1016.0f)

int testingMaxSpeed = 1500;
...
void setup() {
  ...
  Serial.begin(115200);
  SerialBT.begin("Drone do caralho");
  while (!SerialBT) { ; };
  delay(5000);
  bmp280.begin();
  /*
...
void loop() {
  ...
  String speedStr = "";
  String teststr = SerialBT.readString();
  teststr.trim();
  if (teststr == "stop") {
  ...
  if (teststr == "speed") {
    SerialBT.println("input new max speed (pulse width in uS): ");
    while (SerialBT.available() == 0) {};
    speedStr = SerialBT.readString();
    speedStr.trim();
    SerialBT.print("setting new max speed to: ");
    SerialBT.println(speedStr);
